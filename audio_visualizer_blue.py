...
def render(sm, pk, gain, stats, cl_usage):
    ...
    # Each bar = 2 chars wide + 1 char gap => 3 chars per bar
    draw_bars = max(10, (bars_inner + 1) // 3)
    ...
    if show_panels:
        ...
        # Bars (2-char wide bars with 1-char gap, solid cyan)
        for ri in range(draw_h_actual):
            row = draw_h_actual - ri
            ch = []
            chars_used = 0
            for col in range(draw_bars):
                if chars_used + 2 > p1w:
                    break
                h = sm[col]
                p = pk[col]
        ...
        draw_h = max(3, rows - 3)
        grad = make_grad(draw_h)
        fs_bars = max(10, (bar_w + 1) // 3)
        if len(sm) != fs_bars:
            sm = np.zeros(fs_bars)
            pk = np.zeros(fs_bars)
        sm, pk = spectrum(fs_bars, draw_h, gain, sm, pk)
